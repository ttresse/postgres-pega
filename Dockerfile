...
RUN echo deb http://ftp.us.debian.org/debian jessie main >> /etc/apt/sources.list && \
    ...
    apt-get clean && apt-get update && apt-get install -t jessie-backports openjdk-8-jre-headless && apt-get --fix-missing -y --force-yes --no-install-recommends install openjdk-8-jdk-headless && \
